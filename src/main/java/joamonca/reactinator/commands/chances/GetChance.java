...
public class GetChance implements BotCommand {
    ...
    public void execute(CommandDataObject data) {
        ...
        if (channelChance >= 0) {
            reply(event, "current chance in " + channelMention + " is " + channelChance + "%", true);
        } else if (guildChance >= 0) {
            reply(event, "no override set for " + channelMention + ". It uses the guild default (" + guildChance + "%).", true);
        } else {
            reply(event, "failed to retrieve chances.", true);
        }
    }
}
